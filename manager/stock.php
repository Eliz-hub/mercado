...
if (!empty($_GET['search'])) {
    $data = $_GET['search'];
    $sql = "SELECT * FROM produto WHERE cod_produto LIKE '$data' or descricao LIKE '%$data%' or quantidade LIKE '$data' or data_registro LIKE '$data' ORDER BY cod_produto DESC";
}else{
    ...
    <main class="d-flex flex-nowrap">
        <div class="d-flex flex-column flex-shrink-0 bg-light text-center" style="width: 3.5%; min-width: 53px;">
            <ul class="nav nav-pills nav-flush mb-auto">
                ...
                <li class="nav-item">
                    <a href="stock.php" class="nav-link active py-3 border-bottom rounded-0" aria-current="page"
        ...
        <div class="text-center" style="width: 100vw; height: 100%; min-height: 100vh;">
            <h2 style="margin-top: 70px;">Produtos disponíveis em Estoque</h2>
                        ...
                        <tbody style="font-size: 13px">
                            <?php
                                while ($produto_data = mysqli_fetch_assoc($result)) {
                                    ...
                                    echo "<td>".$produto_data['quantidade']."</td>";
                                    echo "<td>".$produto_data['data_registro']."</td>";
                                    echo "<td>
                                        <a class='btn btn-sm btn-primary' style='margin:2px' href='edicao/editar_stock.php?cod_produto=$produto_data[cod_produto]'><img src='../icon/edit.png' width=16 height:16></a>
                                        <a class='btn btn-sm btn-danger' style='margin:2px' href='edicao/delete_onstock.php?cod_produto=$produto_data[cod_produto]'><img src='../icon/lixeira.png' width=16 height:16></a>
                                    </td>";
                                    echo "</tr>";
                                }
                            ?>
...
<script>
    ...
    function searchData(){
        window.location = 'stock.php?search='+search.value;
    }
</script>
